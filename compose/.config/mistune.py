...
class InlineGrammar(object):
    ...
    high_yellow = re.compile(r'^={2}(?=\S)([\s\S]*?\S)={2}')
...
class InlineLexer(object):
    ...
    default_rules = [
        ...
        'linebreak', 'strikethrough', 'high_yellow', 'text'
    ]
    inline_html_rules = [
    ...
    def output_high_yellow(self, m):
        text = self.output(m.group(1))
        return self.renderer.high_yellow(text)
...
class Renderer(object):
    ...
    def high_yellow(self, text):
        """黄色高亮

        :param text: text content
        """
        return '<yellow>%s</yellow>' % text
