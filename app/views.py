...
class IndexView(ListView):
    ...
    def pagination_data(paginator, page, is_paginated):
        ...
        if page_number == 1:
            ...
            # 如果最右边的页码号比最后一页的页码号小，说明当前页右边的连续页码号中不包含最后一页的页码，所以需要显示最后一页的页码号，通过 last 来指示
...
class PostDetailView(DetailView):
    ...
    def get_object(self, queryset=None):
        ...
        post.body = f_text
        post.toc = '' if strip_tags(toc).replace('\n', '') == '' else toc
...
class ArchiveView(IndexView):
    ...
    def get_queryset(self):
        ...
        return super(ArchiveView, self).get_queryset().filter(created_time__year=year, created_time__month=month).order_by('-created_time')
